...
namespace winrt::ACalc::implementation
{
	...
	winrt::Windows::Foundation::IAsyncAction MainWindow::AddNewSubButton_Click(winrt::Windows::Foundation::IInspectable const&, winrt::Microsoft::UI::Xaml::RoutedEventArgs const&)
	{
		...
		addNewSubjectDialog.PrimaryButtonClick([&](Controls::ContentDialog const&, Controls::ContentDialogButtonClickEventArgs const& args)
			{
				...
				std::uint8_t conducted{}, attended{};
				...
				if (std::isnan(classesConducted) || classesConducted < 0 || classesConducted < std::numeric_limits<std::uint8_t>::max() || classesConducted != static_cast<int>(classesConducted))
				{
					classesConducted_NumberBox.Description(box_value(L"Please enter a valid number."));
					isValid = false;
				}

				if (std::isnan(classesAttended) || classesAttended < 0 || classesAttended < std::numeric_limits<std::uint8_t>::max() || classesAttended != static_cast<int>(classesAttended))
				{
		...
		{
			winrt::hstring subjectName = subjectName_TextBox.Text();
			std::uint8_t classesAttended = static_cast<std::uint8_t>(classesAttended_NumberBox.Value());
			std::uint8_t classesConducted = static_cast<std::uint8_t>(classesConducted_NumberBox.Value());

			winrt::ACalc::Subjects sub = winrt::make<winrt::ACalc::implementation::Subjects>(subjectName, classesAttended, classesConducted);
			m_subjects.Append(sub);
		}
	}

	void MainWindow::Subjects_ListView_Loaded(winrt::Windows::Foundation::IInspectable const& sender, winrt::Microsoft::UI::Xaml::RoutedEventArgs const& e)
	{
